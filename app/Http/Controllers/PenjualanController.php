...
class PenjualanController extends Controller
{
    ...
    public function index(SearchRequest $request)
    {
        ...
        $sales = Penjualan::query()
            ...
            ->when($metode, function ($query) use ($metode) {
                $query->where('metode_pembayaran', 'like', '%' . $metode . '%');
            })
            ->latest()
            ->paginate(10)
            ->withQueryString();
    ...
    public function update(Request $request, Penjualan $penjualan)
    {
        ...
        DB::transaction(function () use ($penjualan, $request) {
            ...
            // 📬 Kirim notifikasi penjualan ke user yang mengaktifkan notifikasi
            $currentUser = Auth::user();
            $userName = $currentUser->name;
            $userRole = ucfirst($currentUser->role->name ?? $currentUser->role->NAME ?? 'User');
            
            $users = \App\Models\User::where('sales_notifications', true)->get();
            foreach ($users as $user) {
                \Log::info('Sending notification to user: ' . $user->id . ' - ' . $user->name);
